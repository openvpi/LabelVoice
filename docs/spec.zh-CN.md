...
- filepath：字符串，对应音频文件的路径。可以是绝对路径也可以是相对路径。只能引用本地文件系统的文件。不接受类似`file://`之类的URI标记。
...
- type：字符串，层类型。必须为`text`/`duration`中的一种。
- slices：列表，包含一系列slice对象，表示该层中所有切割标记点。slices中的切点应当允许乱序。

不同类型的层中的分块进一步切割时的行为：

- type为text的层中分块被切断，切割出的两段继承原分块原本承载的内容。
- type为duration时：在程序内，切割后的分块中的内容固定显示为该块的实际长度；
保存时内容会保存至text字段中。但读取时并不作处理，而是自动计算正确的显示内容。
当duration类型的标注层中的分块被切断，切割出的两段自动继承正确的长度。
